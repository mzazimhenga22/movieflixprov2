...
async function extractFileFromFilemoon(ctx, filemoonUrl) {
    ...
    const redirectCheerio = (0, cheerio_1.load)(redirectHtml);
    const iframeUrl = redirectCheerio('iframe').attr('src');
    // console.log(`Iframe URL: ${iframeUrl}`);
    if (!iframeUrl) {
        // console.log('No iframe URL found');
        throw new errors_1.NotFoundError('No iframe URL found');
    }
    ...
    if (!evalMatch) {
        ...
        throw new errors_1.NotFoundError('No packed JavaScript found');
    }
    // console.log('Found packed JavaScript, unpacking...');
    // Unpack the JavaScript
    const unpacked = (0, unpacker_1.unpack)(evalMatch[0]);
    ...
    if (!fileMatch) {
        ...
        throw new errors_1.NotFoundError('No file URL found in unpacked JavaScript');
    }
...
async function comboScraper(ctx) {
    ...
    if (!movieId) {
        ...
        if (altId) {
            ...
            throw new errors_1.NotFoundError(`No content found for ${ctx.media.type === 'movie' ? 'movie' : 'episode'}`);
        }
    }
    ctx.progress(40);
    ...
    const $ajax = (0, cheerio_1.load)(ajaxHtml);
    ...
    if (!filemoonUrl) {
        ...
        if (allServers.length === 0) {
            throw new errors_1.NotFoundError('No streaming servers found');
        }
        throw new errors_1.NotFoundError('Filemoon server not available');
    }
    ctx.progress(60);
    // Extract file URL from Filemoon
    const fileUrl = await extractFileFromFilemoon(ctx, filemoonUrl);
    if (!fileUrl) {
        throw new errors_1.NotFoundError('Failed to extract file URL from streaming server');
    }
    ctx.progress(90);
    return {
        stream: [
...
exports.zoechipScraper = (0, base_1.makeSourcerer)({
    id: 'zoechip',
    name: 'ZoeChip',
    rank: 170,
    disabled: true,
