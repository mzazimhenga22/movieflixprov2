...
exports.warezcdnScraper = void 0;
const base_1 = require("../../../providers/base");
const mixdrop_1 = require("../../../providers/embeds/mixdrop");
const hls_1 = require("../../../providers/embeds/warezcdn/hls");
const mp4_1 = require("../../../providers/embeds/warezcdn/mp4");
const warezplayer_1 = require("../../../providers/embeds/warezcdn/warezplayer");
...
async function getEmbeds(id, servers, ctx) {
    ...
    for (const server of servers.split(',')) {
        await ctx.proxiedFetcher(`/getEmbed.php`, {
            baseUrl: common_1.warezcdnBase,
            headers: {
                Referer: `${common_1.warezcdnBase}/getEmbed.php?${new URLSearchParams({ id, sv: server })}`,
            },
            method: 'HEAD',
            query: { id, sv: server },
        });
        ...
        if (url && server === 'warezcdn') {
            embeds.push({ embedId: hls_1.warezcdnembedHlsScraper.id, url }, { embedId: mp4_1.warezcdnembedMp4Scraper.id, url }, { embedId: warezplayer_1.warezPlayerScraper.id, url });
        }
        else if (url && server === 'mixdrop')
            embeds.push({ embedId: mixdrop_1.mixdropScraper.id, url });
    }
    return { embeds };
}
exports.warezcdnScraper = (0, base_1.makeSourcerer)({
    id: 'warezcdn',
    name: 'WarezCDN',
    disabled: true,
    rank: 115,
    ...
    scrapeMovie: async (ctx) => {
        if (!ctx.media.imdbId)
            throw new errors_1.NotFoundError('This source requires IMDB id.');
        ...
        if (!id || !servers)
            throw new errors_1.NotFoundError('Failed to find episode id');
        ctx.progress(40);
