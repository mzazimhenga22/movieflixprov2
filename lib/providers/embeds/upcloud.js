...
const { AES, enc } = crypto_js_1.default;
...
exports.upcloudScraper = (0, base_1.makeEmbed)({
    id: 'upcloud',
    name: 'UpCloud',
    rank: 200,
    disabled: true,
    async scrape(ctx) {
        ...
        streamRes.tracks.forEach((track) => {
            ...
            if (!type)
                return;
            const language = (0, captions_1.labelToLanguageCode)(track.label.split(' ')[0]);
            if (!language)
                return;
            captions.push({
                id: track.file,
