...
exports.convertPlaylistsToDataUrls = convertPlaylistsToDataUrls;
const hls_parser_1 = require("hls-parser");
async function convertPlaylistsToDataUrls(fetcher, playlistUrl, headers) {
    ...
    const playlist = (0, hls_parser_1.parse)(playlistData);
    if (playlist.isMasterPlaylist) {
        ...
        await Promise.all(playlist.variants.map(async (variant) => {
            ...
            const variantPlaylist = (0, hls_parser_1.parse)(variantPlaylistData);
            variant.uri = `data:application/vnd.apple.mpegurl;base64,${btoa((0, hls_parser_1.stringify)(variantPlaylist))}`;
        }));
    }
    return `data:application/vnd.apple.mpegurl;base64,${btoa((0, hls_parser_1.stringify)(playlist))}`;
}
