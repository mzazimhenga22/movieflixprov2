...
/** Temporary holder for each raw embed before mapping to SourcererEmbed */
interface RawEmbed {
  source: string;
  url: string;
}
...
const universalScraper = async (
  ...
  const sources = await ctx.proxiedFetcher<SourcesResult>(
    `/ajax/embed/episode/${dataId}/sources`,
    {
      baseUrl: vidSrcToBase,
      headers: { referer },
    },
  ...
  const embedArr: RawEmbed[] = []; // ✅ give the array an explicit type

  // collect all raw embeds
  for (const source of sources.result) {
    const sourceRes = await ctx.proxiedFetcher<SourceResult>(
      `/ajax/embed/source/${source.id}`,
      {
        baseUrl: vidSrcToBase,
        headers: { referer },
      },
  ...
  for (const embedObj of embedArr) {
    ...
    if (embedObj.source === 'Filemoon') {
      ...
      const urlWithSubtitles = embedArr.find(
        (v) => v.source === 'Vidplay' && v.url.includes('sub.info'),
      )?.url;
