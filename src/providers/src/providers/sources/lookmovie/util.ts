...
export async function searchAndFindMedia(
  ...
  if (media.type === 'show') {
    ...
    const result = results.find((res: ResultItem) =>
      compareMedia(media, res.title, Number(res.year)),
    );
    return result;
  }
...
export async function scrape(
  ...
  let id: string | null = null; // ✅ allow string OR null
  ...
  if (media.type === 'movie') {
    ...
    const episode = data.episodes?.find((v: episodeObj) =>
      Number(v.season) === Number(media.season.number) &&
